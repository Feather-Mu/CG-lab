...
@ti.func
def in_shadow(p, light_pos):
    """检查点 p 是否在阴影中"""
    shadow_ray_dir = normalize(light_pos - p)
    shadow_ray_origin = p + shadow_ray_dir * 1e-4  # 偏移避免自相交
    ...
    occluded_by_sphere = (0 < t_sph < light_dist)
    in_shadow_result = in_shadow_result or occluded_by_sphere
    
    # 检查与圆锥的阴影遮挡
    t_cone, _ = intersect_cone(shadow_ray_origin, shadow_ray_dir, ti.Vector([1.2, 1.2, 0.0]), -1.4, 1.2)
    occluded_by_cone = (0 < t_cone < light_dist)
    in_shadow_result = in_shadow_result or occluded_by_cone
...
def main():
    window = ti.ui.Window("Blinn-Phong Shading + Hard Shadow", (res_x, res_y))
