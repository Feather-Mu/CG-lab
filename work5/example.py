...
# 1. 直接读取助教打包好的本地模型文件
# ---------------------------------------------------------
# 获取当前 main.py 脚本所在的绝对路径（即 d:/CG-lab/src/work5/）
script_dir = os.path.dirname(os.path.abspath(__file__))
# 拼接成完整的绝对路径
obj_path = os.path.join(script_dir, "cow.obj") 

if not os.path.exists(obj_path):
    raise FileNotFoundError(f"未找到 cow.obj，请确保该文件位于: {obj_path}")

# 准备目标数据与归一化处理
# 建议加上 load_textures=False，防止之后又被 .mtl 或贴图文件坑一遍
